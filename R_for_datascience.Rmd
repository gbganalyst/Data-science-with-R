...
knitr::opts_chunk$set(echo = FALSE, tidy = "styler")
```
...
If you doubt me, please kindly ask Hadley Wickham, Ezekiel ([\@gbganalyst](https://twitter.com/gbganalyst)) and Obisesan (\@obadairo1).
...
1 & Grade & Grade(Test) \\ 
  ...
  3 & Ibadan\_R\_users & Ibadan R users \\ 
...
character.vector <- c('Chidinma', 'Emmanuel', 'Abono', 'Adesanya', 'Ebun')
...
# Short group work
## Tell us what the following line of codes is or are doing
```{r, echo=T, eval=F}
...
# Group work
![Group work](Images/assignment_Capture.JPG)
...
```{r,echo=T, eval=T}
B <- matrix(c(1,-6, 3, -8), nrow = 2, ncol = 2, 
            byrow = TRUE)
...
The function for calculating `eigenvalues` is `eigen()`. Note the function `eigen()` will produce a list as results. You will soon know what a `list()` is in the next next section.
...
## Example
`#` This shows all the R datasets in the package `datasets`.
